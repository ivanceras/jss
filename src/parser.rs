...
fn alpha_or_underscore(ch: char) -> bool {
    pom::char_class::alpha(ch as u8) || underscore(ch)
}

fn alphanum_or_underscore(ch: char) -> bool {
    pom::char_class::alphanum(ch as u8) || underscore(ch)
}

fn underscore(ch: char) -> bool {
    ch == '_'
}
...
pub(crate) fn ident_kebab<'a>() -> Parser<'a, char, String> {
    list(strict_ident(), sym('-')).map(|idents| idents.join("-"))
}

fn ident_key<'a>() -> Parser<'a, char, String> {
    ident_kebab() | strict_ident()
}
...
fn object<'a>() -> Parser<'a, char, Vec<(String, JsonValue)>> {
    let member = ident_key() - space() - sym(':') - space() + call(value);
    let members = list(member, (sym(',') | sym(';')) * space());
    let obj = sym('{') * space() * members - sym('}');
    obj.map(|members| Vec::from_iter(members))
}
...
fn value<'a>() -> Parser<'a, char, JsonValue> {
    (tag("null").map(|_| JsonValue::Null)
        ...
        | strict_ident().map(|text| JsonValue::Str(text))
